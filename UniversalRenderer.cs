    ...
    public sealed class UniversalRenderer : ScriptableRenderer
    {
        ...
        public override void Setup(ScriptableRenderContext context, ref RenderingData renderingData)
        {
            ...
            else if (m_bCustom)
            {
                ...
                int msaaSamples = colorDescriptor.msaaSamples;
                colorDescriptor.width = Screen.width;
                colorDescriptor.height = Screen.height;
                var colorDescriptor1 = colorDescriptor;

                cmd.GetTemporaryRT(m_RenderScale.id, colorDescriptor1, FilterMode.Bilinear);
                ...
                var depthDescriptor = colorDescriptor1;
                depthDescriptor.colorFormat = RenderTextureFormat.Depth;
                depthDescriptor.depthBufferBits = k_DepthStencilBufferBits;
                cmd.ReleaseTemporaryRT(m_ActiveCameraDepthAttachment.id);
                cmd.GetTemporaryRT(m_ActiveCameraDepthAttachment.id, depthDescriptor, FilterMode.Point);
                //cmd.ReleaseTemporaryRT(m_AfterPostProcessColor.id);
